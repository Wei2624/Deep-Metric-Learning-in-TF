function images = load_cropped_images(image_dir, image_list, ...
                                crop_padding, force_square_size,class_ids,training_set)
                            
images = struct();
pos = 0;

if training_set
    image_sum = zeros(224,224,3);
else
    image_mean = csvread('image_mean.csv');
end
...
for i_img = 1:length(image_list)
    ...
    pos = pos + 1;
    if training_set
        image_sum = image_sum + double(img_cropped);
    ...
    images(pos).class_id = class_ids(i_img);
...
if training_set
    image_mean = image_sum/length(image_list);
    save('image_mean.mat', 'image_mean');
end