...
class CNN_Triplet_Metric(object):
    def __init__(self, sess):
        ...
        print("class label has unique:" + str(np.unique(class_id[0:8000]).shape))
        np.savetxt('train_class_label_car196.csv', class_id[0:8000])
        ...
        train_writer = tf.summary.FileWriter('../tensorflow_log/train/', self.sess.graph)
        global_step = 0
        log_step = 0
        batch_size = 100
        ...
        for epoch in range(15):
            ...
            for step in range(int(total_no_train / batch_size)):
                ...
                if (global_step+1)%10==0:
                    train_writer.add_summary(m, log_step)
                    print("Epoch:", '%04d' % (epoch), "Step:", '%04d' % (global_step), \
                            "cost=", "{:.9f}".format(cost), "accu=", "{:.9f}".format(accu))
                    log_step = log_step+1
        ...
        #
        #ptr = 0
        #no_of_batches_train = int(img_data.shape[0] / 100)
        #imean = np.tile(image_mean, (100, 1, 1, 1))
        #for k in range(no_of_batches_train):
        #    print(ptr)
        ...
        #    ptr += 100
        #
        #np.savetxt("../feature/results_tf_64_training_196.csv", results, delimiter=",")
        #print('done extracting from training data')
        #
        #print('read test data')
        #with h5py.File(root_dir+"validation_cars196_256cropped.mat") as f:
        ...
        #class_id = np.asarray(class_id)
        #class_id = class_id[:, 0, 0]
        #print("class label has unique:" + str(np.unique(class_id[0:8100]).shape))
        #np.savetxt('test_class_label_car196.csv',class_id[0:8100])
        #print('testing dataset shape:' + str(img_data.shape))
        #
        #results = np.zeros((8100, 64), dtype=np.float32)
        #
        #ptr = 0
        ...
        #np.savetxt("../feature/results_tf_64_after_training_196.csv", results, delimiter=",")
    
    # print results.shape
    
    def Variables_Dict(self):
    	print('Loading MAT file for pretrained')
    	pretrained_weights = scipy.io.loadmat('../data/tf_ckpt_from_caffe.mat')
