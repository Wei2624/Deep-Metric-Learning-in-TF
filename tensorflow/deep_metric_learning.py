...
def get_feature(img_data):
    ...
    for step in tqdm(range(int(num_sample / args.test_batch_size)+1)):
        ...
        if args.data_augment:
            ...
            img = img_data[ptr:ptr + real_size, :, :, :]
        feature = sess.run([bottleneck],feed_dict={google_net_model.img: img})
...
def read_data(name, image_mean):
    ...
    for i in tqdm(range(num_training_sample)):
        ...
        original_img_data[i,:,:,:] = original_img_data[i,:,:,:]  - np.float32(image_mean)
...
out_layer, bottleneck = retrieval_layer.get_output(google_net_model.google_net_feature,\
...
train_op1 = tf.train.RMSPropOptimizer(learning_rate=0.0001,decay=0.94).minimize(loss_op, var_list=google_net_model.var_dict.values())
